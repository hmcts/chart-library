...
{{- include "hmcts.tests.header.v3" $smokedata }}
spec:
{{ $_ := set $smokedata.Values "tests" $languageValues.smoketests }}
{{- include "hmcts.tests.spec.v3" $smokedata | indent 2 }}
{{- end }}
...
{{- include "hmcts.tests.header.v3" $functionaldata }}
spec:
{{ $_ := set $functionaldata.Values "tests" $languageValues.functionaltests }}
{{- include "hmcts.tests.spec.v3" $functionaldata | indent 2 }}
{{- end }}

{{ if and $languageValues.smoketestscron.enabled (($globals.smoketestscron | default dict).enabled) }}
---
...
{{- include "hmcts.testscron.header.v3" $smokedatacron }}
{{ $_ := set $smokedatacron.Values "tests" $languageValues.smoketestscron }}
{{- include "hmcts.tests.spec.v3" $smokedatacron | indent 10 }}
{{- end }}

{{ if and $languageValues.functionaltestscron.enabled (($globals.functionaltestscron | default dict).enabled) }}
---
...
{{- include "hmcts.testscron.header.v3" $functionaldatacron }}
{{ $_ := set $functionaldatacron.Values "tests" $languageValues.functionaltestscron }}
{{- include "hmcts.tests.spec.v3" $functionaldatacron | indent 10 }}
{{- end }}
