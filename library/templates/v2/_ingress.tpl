{{- define "hmcts.ingress.v2.2.2.tpl" -}}
...
spec:
  ...
  - host: {{ tpl $languageValues.ingressHost $ | lower }}
    http:
      paths:
      {{- ( include "hmcts.additionalPathBasedRoutes.v2.2.2" .) | indent 4 }}
      - path: /
        pathType: Prefix
        backend:
          service:
...
{{- define "hmcts.ingress.v2.2.2" -}}
{{- template "hmcts.util.merge.v2.2.2" (append . "hmcts.ingress.v2.2.2.tpl") -}}
{{- end -}}

{{/*
Additional Path based routes
*/}}
{{- define "hmcts.additionalPathBasedRoutes.v2.2.2" }}
{{- $languageValues := deepCopy .Values -}}
{{- if hasKey .Values "language" -}}
{{- $languageValues = (deepCopy .Values | merge (pluck .Values.language .Values | first) ) -}}
{{- end -}}
