...
spec:
  ...
  - host: {{ tpl $languageValues.ingressHost $ | lower }}
    http:
      paths:
      {{- ( include "hmcts.additionalPathBasedRoutes.v4" .) | indent 4 }}
      - path: /
        pathType: Prefix
        backend:
          service:
...
{{- define "hmcts.ingress.v5" -}}
{{- template "hmcts.util.merge.v2" (append . "hmcts.ingress.v5.tpl") -}}
{{- end -}}

{{/*
Additional Path based routes
*/}}
{{- define "hmcts.additionalPathBasedRoutes.v4" }}
{{- $languageValues := deepCopy .Values -}}
{{- if hasKey .Values "language" -}}
{{- $languageValues = (deepCopy .Values | merge (pluck .Values.language .Values | first) ) -}}
{{- end -}}
