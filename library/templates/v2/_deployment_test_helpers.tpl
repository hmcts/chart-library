...
metadata:
  ...
  labels:
    ...
    app.kubernetes.io/name: {{ template "hmcts.releasename.v3" . }}-{{ .Values.task }}{{ .Values.type }}
{{- end -}}

{{- define "hmcts.tests.header.v3" -}}
apiVersion: v1
kind: Pod
{{ template "hmcts.tests.meta.v3" . }}
...
{{- define "hmcts.testscron.header.v3" -}}
  ...
  jobTemplate:
    spec:
      ...
      template:
        metadata:
          labels:
            app.kubernetes.io/name: {{ template "hmcts.releasename.v3" . }}-{{ .Values.task }}testscron
            {{- if and .Values.aadIdentityName (not .Values.useWorkloadIdentity) }}
            aadpodidbinding: {{ .Values.aadIdentityName }}
            {{- end }}
        spec:
...
- name: vault-{{ $key }}
  csi:
    ...
    readOnly: true
    volumeAttributes:
      secretProviderClass: {{ template "hmcts.releasename.v3" $root }}-tests-{{ $key }}
{{- end }}
{{- end }}
securityContext:
  runAsUser: 1000
