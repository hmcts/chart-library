{{- define "hmcts.tests.meta.v2.2.2" -}}
metadata:
  ...
  labels:
    ...
    app.kubernetes.io/name: {{ template "hmcts.releasename.v2.2.2" . }}-{{ .Values.task }}{{ .Values.type }}
{{- end -}}

{{- define "hmcts.tests.header.v2.2.2" -}}
apiVersion: v1
kind: Pod
{{ template "hmcts.tests.meta.v2.2.2" . }}
...
{{- define "hmcts.testscron.header.v2.2.2" -}}
  ...
  jobTemplate:
    spec:
      ...
      template:
        metadata:
          labels:
            app.kubernetes.io/name: {{ template "hmcts.releasename.v2.2.2" . }}-{{ .Values.task }}testscron
            {{- if and .Values.aadIdentityName (not .Values.useWorkloadIdentity) }}
            aadpodidbinding: {{ .Values.aadIdentityName }}
            {{- end }}
        spec:
...
{{- define "hmcts.tests.spec.v2.2.2" -}}
...
- name: vault-{{ $key }}
  csi:
    ...
    readOnly: true
    volumeAttributes:
      secretProviderClass: {{ template "hmcts.releasename.v2.2.2" $root }}-tests-{{ $key }}
{{- end }}
{{- end }}
securityContext:
  runAsUser: 1000
