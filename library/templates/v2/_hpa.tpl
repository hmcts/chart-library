...
spec:
  ...
  minReplicas: {{ $languageValues.autoscaling.minReplicas | default $languageValues.replicas | default 1 }}
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
  ...
  - type: Resource
    resource:
      ...
      target:
        type: Utilization
        averageUtilization: {{ $languageValues.autoscaling.cpu.averageUtilization | default 80 }}
  {{- end }}
  {{- if (($languageValues.autoscaling.memory | default dict).enabled) }}
  {{/*type: Resource is rendered at the bottom of the resource block in the template.*/}}
  - type: Resource
    resource:
      name: memory
      target:
        type: Utilization
        averageUtilization: {{ $languageValues.autoscaling.memory.averageUtilization | default 80 }}
{{- end }}
{{- end }}
{{- end }}
{{- end }}
